import { Logo } from "./Logo";

export function Header({ onConnect }: { onConnect: () => void }) {
  return (
    <header className="fixed top-0 z-40 w-full border-b border-border/40 bg-surface/80 backdrop-blur-xl">
      <div className="mx-auto flex h-14 max-w-6xl items-center justify-between gap-4 px-4 sm:px-6">
        <a href="#" className="flex min-w-0 shrink-0 items-center gap-2.5">
          <span className="inline-flex h-7 w-7 shrink-0 items-center justify-center overflow-hidden rounded-lg">
            <Logo className="h-7 w-7" />
          </span>
          <span className="truncate text-sm font-semibold tracking-tight text-white">
            Synapsee <span className="font-medium text-cyan">IA</span>
          </span>
        </a>

        <nav className="hidden items-center gap-8 text-sm text-slate-500 md:flex">
          <a href="#como-funciona" className="transition hover:text-white">
            Como funciona
          </a>
          <a href="#entregaveis" className="transition hover:text-white">
            Capacidades
          </a>
          <a href="#planos" className="transition hover:text-white">
            Acesso
          </a>
        </nav>

        <button
          type="button"
          onClick={onConnect}
          className="shrink-0 rounded-lg cyan-gradient px-4 py-2 text-sm font-semibold text-surface transition hover:brightness-110"
        >
          Solicitar acesso
        </button>
      </div>
    </header>
  );
}
